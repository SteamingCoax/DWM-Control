...
**Request frame**
```
proto=2 type=cmd cmd=<command> [req=<id>] [other keys]\r\n
```

**Successful response**
```
proto=2 type=resp status=ok cmd=<command> [req=<id>] <payload keys>\r\n
```

**Error response**
```
proto=2 type=err status=error cmd=<command> [req=<id>] code=<error> msg=<detail>\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=pwr.get req=101 met=avg\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=pwr.get req=101 met=avg value=1.234567 elem=1 etype=30ua eval=0.033 range=2x\r\n
```
...
- `range` — ADC gain range in use (`1x`, `2x`, or `4x`)

**Error example** (unknown metric name)
```
proto=2 type=err status=error cmd=pwr.get req=101 code=ERR_UNKNOWN_METRIC msg=unsupported_metric\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=pwr.snap req=102\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=pwr.snap req=102 d=1.234567,1.180000,2.100000,2.500000,0.900000,0.043210,3299.00,5.0 elem=1 etype=30ua eval=250.000 range=2x\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=pwr.info req=103\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=pwr.info req=103 elem=1 etype=30ua eval=0.033 range=2x\r\n
```

- `elem` — currently selected element number (integer) (1-8)
- `etype` — Output type of that element (`30ua` or `100ua`)
- `eval` — Rating of that element in watts (float) (e.g: 0.1, 250, or 50000)
- `range` — ADC gain range (`1x`, `2x`, or `4x`)
...
**Request**
```
proto=2 type=cmd cmd=sys.id req=104\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.id req=104 uid=1A2B3C4D5E6F7A8B9C0D1E2F dname=bench_meter_a\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.fw req=105\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.fw req=105 fver=FW:_2.6.0_-_COMMS\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.nget req=105\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.nget req=105 dname=bench_meter_a\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.nset req=106 name=lab_supply_monitor\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.nset req=106 dname=lab_supply_monitor\r\n
```
...
**Error example**
```
proto=2 type=err status=error cmd=sys.nset req=106 code=ERR_SETTING_REJECTED msg=invalid_name\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.cmds req=107\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.cmds req=107 cmds=pwr.get,pwr.snap,pwr.info,sys.id,sys.fw,sys.nget,sys.nset,sys.cmds,cfg.get,cfg.elem,cfg.elems,cfg.set,sys.dfu,sys.save,sys.rst\r\n
```
---
...
| `range` | integer | ADC gain range (`0` = 1×, `1` = 2×, `2` = 4×) |
...
**Request**
```
proto=2 type=cmd cmd=cfg.get req=108 key=bright\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=cfg.get req=108 key=bright val=7\r\n
```

**Element rating request**
```
proto=2 type=cmd cmd=cfg.get req=109 key=eval elem=3\r\n
```

**Element rating response**
```
proto=2 type=resp status=ok cmd=cfg.get req=109 key=eval elem=3 val=100.000000\r\n
```

**Error example**
```
proto=2 type=err status=error cmd=cfg.get req=108 code=ERR_BAD_ENUM msg=unknown_setting_key\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=cfg.elems req=109\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=cfg.elems req=109 e1v=100.000 e1t=30ua e2v=100.000 e2t=30ua e3v=100.000 e3t=30ua e4v=100.000 e4t=30ua e5v=100.000 e5t=30ua e6v=100.000 e6t=30ua e7v=100.000 e7t=30ua e8v=100.000 e8t=30ua\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=cfg.elem req=110 elem=3\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=cfg.elem req=110 elem=3 eval=100.000000 etype=30ua\r\n
```
...
**Error example**
```
proto=2 type=err status=error cmd=cfg.elem req=110 code=ERR_VALUE_RANGE msg=elem_out_of_range\r\n
```
...
| `range` | integer | 0–2 | ADC gain range (`0` = 1×, `1` = 2×, `2` = 4×) |
| `avgw` | float | 0.5–10.0 | Averaging window in seconds |

**Request**
```
proto=2 type=cmd cmd=cfg.set req=108 key=bright val=7\r\n
```

**Response** — echoes the key and value that was stored:
```
proto=2 type=resp status=ok cmd=cfg.set req=108 key=bright val=7\r\n
```

**Element type request**
```
proto=2 type=cmd cmd=cfg.set req=109 key=etype elem=3 val=100ua\r\n
```

**Element type response**
```
proto=2 type=resp status=ok cmd=cfg.set req=109 key=etype elem=3 val=100ua\r\n
```

**Element rating request**
```
proto=2 type=cmd cmd=cfg.set req=110 key=eval elem=3 val=250.000000\r\n
```

**Element rating response**
```
proto=2 type=resp status=ok cmd=cfg.set req=110 key=eval elem=3 val=250.000000\r\n
```

**Error examples**
```
proto=2 type=err status=error cmd=cfg.set req=108 code=ERR_BAD_ENUM msg=unknown_setting_key\r\n
proto=2 type=err status=error cmd=cfg.set req=108 code=ERR_VALUE_RANGE msg=bright_out_of_range\r\n
proto=2 type=err status=error cmd=cfg.set req=108 code=ERR_BAD_VALUE msg=avgw_not_a_number\r\n
proto=2 type=err status=error cmd=cfg.set req=109 code=ERR_MISSING_KEY msg=missing_elem\r\n
proto=2 type=err status=error cmd=cfg.set req=109 code=ERR_BAD_ENUM msg=etype_invalid\r\n
proto=2 type=err status=error cmd=cfg.set req=110 code=ERR_BAD_VALUE msg=eval_not_a_number\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.dfu req=109\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.dfu req=109\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.save req=110\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.save req=110\r\n
```

**Error example**
```
proto=2 type=err status=error cmd=sys.save req=110 code=ERR_SETTING_REJECTED msg=save_failed\r\n
```
...
**Request**
```
proto=2 type=cmd cmd=sys.rst req=111\r\n
```

**Response**
```
proto=2 type=resp status=ok cmd=sys.rst req=111\r\n
```
...
| `ERR_BAD_ENUM` | A key had an unsupported value (e.g. `proto=3`) |
